    ...
    public class InlineArgumentsBlock : AbstractEmitterBlock
    {
        ...
        protected virtual void EmitInlineExpressionList(ArgumentsInfo argsInfo, string inline, bool asRef = false, bool isNull = false, bool? definition = null)
        {
            ...
            if (asRef)
            {
                ...
                if (withoutTypeParams &&
                    ((!this.Method.IsStatic || this.Method.IsExtensionMethod && this.TargetResolveResult is ThisResolveResult)
                    || (this.TargetResolveResult is ThisResolveResult || this.TargetResolveResult == null) 
                    || (inline.Contains("{this}") || this.Method.IsStatic || this.Method.IsExtensionMethod && inline.Contains("{" + this.Method.Parameters.First().Name + "}"))))
                {
